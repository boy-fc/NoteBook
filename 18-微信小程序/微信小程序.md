...
+ 注册change事件并且将id传递进事件函数内部
  ...
  ```html
  <van-stepper data-id="{{item.id}}" value="{{ item.count }}" bind:change="countChange"  data-id='{{item.id}}' />
...
+ 给删除按钮注册点击事件
  ...
  ```js
  <view slot="right" class="cell_right"  @tap='deleteGoods({{item.id}})'>删除</view>
  ```
...
+ ![](微信小程序.assets/image-20191110205203516.png)
...
- 设置每次进入购物车页面都要设置tabbar的徽章(cart.js)
